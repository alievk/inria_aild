...
def custom_load_state_dict(model, state_dict, num_towers):
    def get_load_names():
        result = []
        for name, _ in model.state_dict().items():
            name_parts = name.split('.')
            if 'last_linear' in name_parts:
                return result
            for i, tower_name in enumerate(model.tower_names):
                if tower_name in name_parts:
                    if i + 1 > num_towers:
                        return result
            result.append(name)
        return result
...
def get_blocks(module, n):
    blocks = []
    for i in range(n):
        blocks.append(copy(module))
...
class InceptionResNetV2(nn.Module):
    ...
    def __init__(self, num_classes, tower_sizes):
        ...
        self.repeat_1 = get_blocks(Block17(scale=0.10), tower_sizes[1])
...
def inceptionresnetv2(num_classes, tower1_size=10, tower2_size=20, tower3_size=9, pretrained=True):
    ...
    if pretrained:
        ...
        if tower1_size != signature.parameters['tower1_size'].default:
            ...
            num_towers = 1
        elif tower3_size != signature.parameters['tower3_size'].default:
            num_towers = 2
        else:
            num_towers = 3
        custom_load_state_dict(
            model,
            model_zoo.load_url(pretrained_settings['url']),
            num_towers
        )
    model.input_space = pretrained_settings['input_space']
    model.input_range = pretrained_settings['input_range']
    model.mean = pretrained_settings['mean']
    model.std = pretrained_settings['std']
